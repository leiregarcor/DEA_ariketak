...
import java.util.HashMap;
	...
	private Integer evaluar(BinaryTreeNode<InfoElemExp> a, HashMap<String, Integer> tHash) {
		// Pre: a no es vacío
		if (!a.element.operador){ //si es un número
			//puede estar en el thash o no, lo comprobamos
			if (tHash.containsKey(a.element.elem)){ //está en el thash y su valor es :
				return tHash.get(a.element.elem);
			}
			else { //no está en el thash, por lo que su valor será 0
				return 0;
			}
		}
		else{ //si es un operador, necesitams la evaluación de lo que hay a izq y drch para poder proceder
			int ezk = evaluar(a.right,tHash);
			int esk = evaluar(a.left,tHash);
			if (a.element.elem.equals("*")){  //eragigaia * da
				return ezk*esk;
			}
			else{ //eragigaia + da
				return esk+ezk;
